...
struct PostCard: View {
    ...
    private var publicActions: some View { HStack(spacing: 3) { Button { store.like(post) } label: { Label("\(post.likes)", systemImage: post.liked ? "heart.fill" : "heart").labelStyle(.titleAndIcon).lineLimit(1).frame(width: 52, height: 25) }.buttonStyle(.bordered).tint(post.liked ? .pink : .gray).frame(width: 52, height: 25); Button { store.favorite(post) } label: { Label("\(post.favorites)", systemImage: store.favorites.contains(post.id) ? "bookmark.fill" : "bookmark").labelStyle(.titleAndIcon).lineLimit(1).frame(width: 52, height: 25) }.buttonStyle(.bordered).tint(store.favorites.contains(post.id) ? .orange : .gray).frame(width: 52, height: 25); Button { store.selectedPost = post } label: { Label("\(post.comments)", systemImage: "message").labelStyle(.titleAndIcon).lineLimit(1).frame(width: 52, height: 25) }.buttonStyle(.bordered).tint(.gray).frame(width: 52, height: 25); ShareLink(item: post.text) { Image(systemName: "arrowshape.turn.up.right").frame(width: 52, height: 25) }.buttonStyle(.bordered).tint(.gray).frame(width: 52, height: 25) }.font(.caption2).frame(maxWidth: .infinity, alignment: .center) }
