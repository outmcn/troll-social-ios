...
struct SocialTabBar: View {
    ...
    var body: some View {
        ...
        }.padding(.horizontal, 8).padding(.top, 8).padding(.bottom, 6).background(.ultraThinMaterial)
    }
...
struct FilterRow: View { var body: some View { HStack { ForEach(["推荐", "关注", "生活", "兴趣"], id: \.self) { item in Text(item).font(.caption.bold()).padding(.horizontal, 13).padding(.vertical, 8).background(item == "推荐" ? Color.orange : Color.white).foregroundColor(item == "推荐" ? .white : .secondary).clipShape(Capsule()) } } } }
