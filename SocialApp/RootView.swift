...
struct Header: View { let title: String?; let subtitle: String?; init(title: String? = nil, subtitle: String? = nil) { self.title = title; self.subtitle = subtitle }; var body: some View { VStack(alignment: .leading, spacing: 5) { if let title { Text(title).font(.system(size: 29, weight: .bold)) }; if let subtitle { Text(subtitle).font(.subheadline).foregroundColor(.secondary) } } } }
...
struct PlazaTab: View { @EnvironmentObject var store: SocialStore; var body: some View { NavigationStack { ScrollView { VStack(alignment: .leading, spacing: 14) { HStack { Spacer(); Image(systemName: "magnifyingglass") }; Text("发现大家正在分享的内容").foregroundColor(.secondary); FilterRow(); ForEach(store.posts) { post in PostCard(post: post, allowsDelete: false, myMode: false) } }.padding(18) }.background(Color(.systemGroupedBackground)) }.navigationTitle("广场").navigationBarTitleDisplayMode(.inline) } }
