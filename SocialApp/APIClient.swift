...
struct RemotePost: Codable, Identifiable {
    let id: String; let author: String; let handle: String; let authorID: String; let ipRegion: String; let text: String; var likes: Int; let comments: Int; let favorites: Int; let createdAt: String?
    enum CodingKeys: String, CodingKey { case id, author, handle; case authorID = "author_id"; case ipRegion = "ip_region"; case text, likes, comments, favorites; case createdAt = "created_at" }
    init(from decoder: Decoder) throws { let c = try decoder.container(keyedBy: CodingKeys.self); id = try c.decode(String.self, forKey: .id); author = try c.decode(String.self, forKey: .author); handle = try c.decodeIfPresent(String.self, forKey: .handle) ?? ""; authorID = try c.decodeIfPresent(String.self, forKey: .authorID) ?? ""; ipRegion = try c.decodeIfPresent(String.self, forKey: .ipRegion) ?? ""; text = try c.decode(String.self, forKey: .text); likes = try c.decode(Int.self, forKey: .likes); comments = try c.decode(Int.self, forKey: .comments); favorites = try c.decodeIfPresent(Int.self, forKey: .favorites) ?? 0; createdAt = try c.decodeIfPresent(String.self, forKey: .createdAt) }
}
