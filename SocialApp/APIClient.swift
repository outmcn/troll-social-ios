...
struct User: Codable {
    let id: String; var userID: String; var displayName: String; var username: String; var role: String; var bio: String; var avatar: String; var ipRegion: String
...
struct RemotePost: Codable, Identifiable {
    let id: String; let author: String; let handle: String; let authorID: String; let ipRegion: String; let text: String; var likes: Int; let comments: Int; let favorites: Int; let liked: Bool; let favorited: Bool; let createdAt: String?
    enum CodingKeys: String, CodingKey { case id, author, handle; case authorID = "author_id"; case ipRegion = "ip_region"; case text, likes, comments, favorites, liked, favorited; case createdAt = "created_at" }
    init(from decoder: Decoder) throws { let c = try decoder.container(keyedBy: CodingKeys.self); id = try c.decode(String.self, forKey: .id); author = try c.decode(String.self, forKey: .author); handle = try c.decodeIfPresent(String.self, forKey: .handle) ?? ""; authorID = try c.decodeIfPresent(String.self, forKey: .authorID) ?? ""; ipRegion = try c.decodeIfPresent(String.self, forKey: .ipRegion) ?? ""; text = try c.decode(String.self, forKey: .text); likes = try c.decodeIfPresent(Int.self, forKey: .likes) ?? 0; comments = try c.decodeIfPresent(Int.self, forKey: .comments) ?? 0; favorites = try c.decodeIfPresent(Int.self, forKey: .favorites) ?? 0; liked = try c.decodeIfPresent(Bool.self, forKey: .liked) ?? false; favorited = try c.decodeIfPresent(Bool.self, forKey: .favorited) ?? false; createdAt = try c.decodeIfPresent(String.self, forKey: .createdAt) }
}
